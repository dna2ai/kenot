...
import (
	...
	"net/http"
	"bytes"
	"github.com/dna2ai/kenot/storage"
	"github.com/dna2ai/kenot/web"
)
...
func parseEnv() (s *ServerConfig) {
	s = &ServerConfig{}
	s.debug = len(os.Getenv("KENOT_DEBUG")) > 0
	s.serverStaticDir = os.Getenv("KENOT_STATIC_DIR")
	s.serverHttpsCADir = os.Getenv("KENOT_HTTPS_CA_DIR")

	serverHost := os.Getenv("KENOT_HOST")
	serverPort, _ := strconv.Atoi(os.Getenv("KENOT_PORT"))
	if serverPort <= 0 {
		serverPort = 8080
	}
	...
	web.DefaultAuth.FileName = os.Getenv("KENOT_BASIC_AUTH")
